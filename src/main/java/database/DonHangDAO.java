...
public class DonHangDAO implements DAOInterface<DonHang> {
	...
	public ArrayList<DonHang> selectAll() {
		...
		try {
			...
			while (rs.next()) {
				...
				String trangThaiThanhToan = rs.getNString(7);
	...
	public DonHang selectById(DonHang t) {
		...
		try {
			...
			while (rs.next()) {
				...
				KhachHang khachHang = new KhachHangDAO()
						.selectById(new KhachHang(maKH, "", "", "", "", "", "", "", null, "", "", false));
				ketQua = new DonHang(maDH, khachHang, diaChiNguoiMua, diaChiNhanHang, trangThai, hinhThucThanhToan,
						trangThaiThanhToan, soTienDaThanhToan, soTienConThieu, ngayDatHang, ngayGiaoHang);
			}
			JDBCUtil.closeConnection(con);
		} catch (SQLException e) {
	...
	public int insert(DonHang t) {
		int kq = 0;
		Connection con = JDBCUtil.getConnection();
		String sql = "INSERT INTO donhang(madonhang, khachhang, diachinguoimua, diachinguoinhan, trangthai, hinhthucthanhtoan, trangthaithanhtoan, tienthanhtoan, tienthieu, ngaydathang, ngaygiaohang)"
				+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		try {
			PreparedStatement st = con.prepareStatement(sql);
			st.setString(1, t.getMaDonHang());
	...
	public int update(DonHang t) {
		...
		String sql = "UPDATE donhang" + " SET " + "khachhang=?" + ", diachinguoimua=?" + ",diachinguoinhan=?"
				+ ",trangthai=?" + ",hinhthucthanhtoan=?" + ",trangthaithanhtoan=?" + ",tienthanhtoan=?"
				+ ",tienthieu=?" + ",ngaydathang=?" + ",ngaygiaohang=?" + " WHERE madonhang=?";
		try {
			PreparedStatement st = con.prepareStatement(sql);
			st.setString(1, t.getMaDonHang());
			st.setString(2, t.getKhachHang().getMaKhachHang());
